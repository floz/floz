 ...
 */
package fr.tilzy.isometric.graphics 
{
	...
	public class IsoDrawing 
	{
		...
		public static function getRectPath( width:Number = 0, length:Number = 0, height:Number = 0, origin:Point3D = null ):GraphicsPath
		{
			...
			geometry[ 0 ] = IsoMath.isoToScreen( origin.x, origin.y, origin.z );
			
			if ( width > 0 && length > 0 && height <= 0 )
			{
				geometry[ 1 ] = IsoMath.isoToScreen( origin.x + width, origin.y, origin.z );
				geometry[ 2 ] = IsoMath.isoToScreen( origin.x + width, origin.y + length, origin.z );
				geometry[ 3 ] = IsoMath.isoToScreen( origin.x, origin.y + length, origin.z );
			}
			else if ( width > 0 && length <= 0 && height > 0 )
			{
				geometry[ 1 ] = IsoMath.isoToScreen( origin.x + width, origin.y, origin.z );
				geometry[ 2 ] = IsoMath.isoToScreen( origin.x + width, origin.y, origin.z + height );
				geometry[ 3 ] = IsoMath.isoToScreen( origin.x, origin.y, origin.z + height );
			}
			else if ( width <= 0 && length > 0 && height > 0 )
			{
				geometry[ 1 ] = IsoMath.isoToScreen( origin.x, origin.y + length, origin.z );
				geometry[ 2 ] = IsoMath.isoToScreen( origin.x, origin.y + length, origin.z + height );
				geometry[ 3 ] = IsoMath.isoToScreen( origin.x, origin.y, origin.z + height );
			}
			else return null;
