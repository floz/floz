...
package step03
{
	...
	public class Main extends Sprite
	{
		...
		private function mouseDownHandler(e:MouseEvent):void 
		{
			//_bx = 0; 
			//_by = 0;
			_bx = stage.mouseX - _center.x;
			_by = stage.mouseY - _center.y;
		...
		{
			var dx:Number = ( ( _dx - _bx ) - _center.x ) * .2;
			var dy:Number = ( ( _dy - _by ) - _center.y ) * .2;
			_center.x += dx;
			_center.y += dy;
			...
			for ( var i:int; i < n; ++i )
			{
				...
				dx = ( _dx - _bx ) - _dots[ i ].x;
				dy = ( _dy - _by ) - _dots[ i ].y;
