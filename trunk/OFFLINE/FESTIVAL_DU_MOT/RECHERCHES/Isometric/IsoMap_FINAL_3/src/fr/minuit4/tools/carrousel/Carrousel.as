 ...
 * @version 		1.0
 * Date				Février 2009
 * Description		Carrousel ActionScript 3.0 sans utilisation d'API 3D (FlashPlayer 10 ou library OpenSource Pv3D ou autre)
 * @example
 * 
 ...
 * 
 */
package fr.minuit4.tools.carrousel 
{
	...
	public class Carrousel extends Sprite
	{
		...
		public function launch():void
		{
			...
			var r:Number;
			var a:Number;
			for (i = 0; i < _maxItem; i+=1)
			{
				s = _library[i];
				r = _angle + (360 / (_maxItem)) * i;
				a = (r) * (Math.PI / 180);
		...
		private function onFrame(e:Event):void 
		{
			...
			for (i = 0; i < _maxItem; i+=1)
			{
				s = this.getChildAt(i) as Item;
				difX = int((this.mouseX) * .05);
				s.angle += difX;
				if (s.angle > 360)
					s.angle = s.angle - 360;
				else if (s.angle <= 0)
