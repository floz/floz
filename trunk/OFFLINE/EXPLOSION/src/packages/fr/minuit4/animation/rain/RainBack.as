...
package fr.minuit4.animation.rain
{
	...
	public class RainBack extends EventDispatcher
	{
		...
		{
			if ( !RainBack._allowInstance ) throw new Error( "Cette classe ne peut pas être instanciée. Veuillez appeller la méthode static apply()" );
			RainBack._allowInstance = false;
			...
			RainBack._instances[ this ] = true;
			RainBack._numExplosions++;
		...
		{
			if ( !RainBack._instances[ this ] ) return;
			
			delete RainBack._instances[ this ];
			RainBack._numExplosions--;
		...
		static public function apply( target:DisplayObject ):RainBack
		{
			RainBack._allowInstance = true;
			return new RainBack( target );
		}
		
		static public function get numExplosions():int
		{
			return RainBack._numExplosions;
		}
