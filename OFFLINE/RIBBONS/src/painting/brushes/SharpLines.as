...
package painting.brushes 
{
	...
	public class SharpLines extends Sprite implements IBrush
	{
		...
		public function SharpLines( color:uint, linesCount:int, spacing:Number, diffX:Number = 0, diffY:Number = 0 ) 
		{
			this._color = color;
		...
		public function update( mx:Number, my:Number ):void
		{
			...
			while ( piece )
			{
				...
				_datas[ 0x0 ] = piece.x1;
				_datas[ 0x1 ] = piece.y1 + _spacing / ( piece.count * .9 );
				
				_datas[ 0x2 ] = piece.x2;
				_datas[ 0x3 ] = piece.y2 + _spacing / ( piece.count * .9 );
